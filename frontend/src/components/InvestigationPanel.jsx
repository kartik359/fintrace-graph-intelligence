...
export default function InvestigationPanel({
  ...
  return (
    <div className="h-72 border-t border-white/10 bg-slate-950/95 backdrop-blur-xl flex flex-col z-20 shrink-0">
      {/* Tab Navigation Header */}
      <div className="flex items-center justify-between border-b border-white/10 px-4 bg-slate-900/60">
        <div className="flex items-center gap-1 py-1">
          <button
            onClick={() => { setActiveTab('ubo'); runUBOAnalysis(selectedTargetCompany); }}
          ...
          >
            ...
            1. UBO Multi-Hop Traversal
          </button>

          <button
            onClick={() => { setActiveTab('circular'); runCircularScan(); }}
            className={`px-3 py-2 rounded-lg text-xs font-semibold flex items-center gap-2 transition-all ${
              activeTab === 'circular'
                ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 shadow-sm'
                : 'text-slate-400 hover:text-slate-200'
          ...
          >
            ...
            2. Circular Wash-Trading Loops
          </button>

          <button
            onClick={() => { setActiveTab('sanctions'); runSanctionCheck(sanctionStartEntity); }}
            className={`px-3 py-2 rounded-lg text-xs font-semibold flex items-center gap-2 transition-all ${
              activeTab === 'sanctions'
                ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 shadow-sm'
                : 'text-slate-400 hover:text-slate-200'
          ...
          >
            ...
            3. Sanction Proximity Path
          </button>

          <button
            onClick={() => { setActiveTab('nominees'); loadNomineeClusters(); }}
            className={`px-3 py-2 rounded-lg text-xs font-semibold flex items-center gap-2 transition-all ${
              activeTab === 'nominees'
                ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 shadow-sm'
                : 'text-slate-400 hover:text-slate-200'
        ...
        <button
          ...
          className="text-[11px] text-slate-400 hover:text-white px-2 py-1 rounded bg-slate-800/80 border border-white/5"
        >
          Clear Highlights
        </button>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 h-full">
            <div className="space-y-3">
              <label className="text-xs font-semibold text-slate-300 block">
                Target Entity for Beneficial Ownership Discovery:
              </label>
              <div className="flex gap-2">
                <select
                  ...
                  className="flex-1 rounded-lg px-3 py-2 text-xs bg-slate-900 border border-white/10 text-white"
                >
                  <option value="comp-kensington-sovereign">Kensington Sovereign Properties Ltd (Layer 6 - UK Asset)</option>
                  <option value="comp-albion-prime">Albion Prime Real Estate Ltd (Layer 5 - UK Operating)</option>
                  <option value="comp-aethelgard-cap">Aethelgard Capital Management Ltd (Layer 4 - Jersey)</option>
                  <option value="comp-beacon-uk">Beacon UK Diagnostics Ltd (Clean Enterprise Benchmark)</option>
                </select>
                <button
                  onClick={() => runUBOAnalysis(selectedTargetCompany)}
                  className="px-3 py-2 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-semibold flex items-center gap-1 shrink-0"
                >
                  <Play className="w-3.5 h-3.5" /> Run Cypher
                </button>
              </div>
              <div className="p-3 rounded-lg bg-slate-900/90 border border-white/5 text-xs text-slate-400">
                <span className="font-semibold text-slate-200 block mb-1">💡 openCypher Advantage:</span>
                Evaluates arbitrary ownership depths <code className="text-cyan-300">{"(p:Person)-[:OWNS*1..6]->(c:Company)"}</code> without rigid multi-table self joins.
              </div>
            </div>

            {/* Results Table & Chain */}
            <div className="lg:col-span-2 glass-panel p-3 overflow-y-auto max-h-52">
                    ...
                    <div key={idx} className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                          <span className="text-xs text-slate-400">Traversal Depth: <strong className="text-white">{item.hopCount} Hops</strong></span>
                        ...
                        {item.chainNames.map((name, i) => (
                          <React.Fragment key={i}>
                            <span 
                              ...
                              className="px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 border border-white/10 text-xs text-slate-200 font-medium cursor-pointer transition-colors"
                            >
                              {name}
                            </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-400">
                Detects closed transaction loops <code className="text-amber-300">{"(a:BankAccount)-[:TRANSFERRED*3..6]->(a)"}</code> where funds return to the sender to create artificial turnover.
              </p>
              <button
                onClick={runCircularScan}
                className="w-full mt-2 px-3 py-2 rounded-lg bg-amber-600 hover:bg-amber-500 text-white text-xs font-semibold flex items-center justify-center gap-2"
              >
                      ...
                      <div className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-amber-200">
                            Closed Loop Detected: {loop.loopLength} Transactions
                          </span>
                        </div>
                        <span className="font-mono font-bold text-amber-300">
                          Total Laundered: ${(loop.totalVolume / 1000000).toFixed(2)}M USD
                        </span>
                      </div>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px]">
                        {loop.transactions.map((tx, tIdx) => (
                          <div key={tIdx} className="bg-slate-900/80 p-1.5 rounded border border-white/5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="space-y-3">
              ...
              <select
                ...
                className="w-full rounded-lg px-3 py-2 text-xs bg-slate-900 border border-white/10 text-white"
              >
              ...
              <button
                ...
                className="w-full px-3 py-2 rounded-lg bg-red-600 hover:bg-red-500 text-white text-xs font-semibold flex items-center justify-center gap-1.5"
              >
              ...
              {sanctionResults?.paths?.length > 0 ? (
                <div className="space-y-3">
                  {sanctionResults.paths.map((p, idx) => (
                    <div key={idx} className="p-3 rounded-lg bg-red-950/30 border border-red-800/40 space-y-2">
                      <div className="flex items-center justify-between text-xs">
                        <span className="font-bold text-red-300">
                          Shortest Path: {p.distance} Degrees of Separation to Sanction
                        </span>
                        <span className="badge-critical text-[10px] px-2 py-0.5 rounded-full font-bold">
                          HIGH RISK EXPOSURE
                        </span>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-300">
                Shared Nominee Directors & Mass Shell Company Hubs (Threshold &ge; 2)
              </span>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-48 overflow-y-auto">
              {nomineeClusters?.clusters?.map((cluster, idx) => (
                <div key={idx} className="glass-panel p-3 border-purple-800/40 space-y-2">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="px-2 py-0.5 rounded bg-purple-950 text-purple-300 font-mono text-[10px] font-bold">
                      {cluster.companyCount} Connected Shells
                    </span>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 h-full">
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full rounded-lg px-2.5 py-1.5 text-xs bg-slate-900 border border-white/10 text-slate-200"
              >
            ...
            <div className="lg:col-span-2 glass-panel p-3 overflow-y-auto max-h-52 font-mono text-xs">
              {cypherOutput ? (
                <div>
                  <div className="flex items-center justify-between pb-2 border-b border-white/10 mb-2 text-[11px] text-slate-400">
